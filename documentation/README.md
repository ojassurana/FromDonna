...
**User DMs `@fromdonna_bot` → gateway Worker + D1 → dedicated E2B Hermes → LLM proxy (capability) + API proxy (Exa stub) + composio-proxy MCP (per-user Bearer for Gmail/etc.) → reply via Bot API proxy; after use, Worker may pull a runtime checkpoint to R2 if the box is replaced.**
