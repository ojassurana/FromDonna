...
version: 0.1.0
author: Matt Maximo (MattMaximo), Donna
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, donna-agent]
---

# Grok Build CLI — Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the agent terminal. Grok
...
1. Prepare stable input files first with agent tools (`read_file`,
...
2. **Don't conflate the agent's xAI auth with the `grok` CLI's auth.** The agent's
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume the agent's xAI auth carries
   over.
