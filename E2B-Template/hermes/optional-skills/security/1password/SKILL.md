...
version: 1.0.0
author: arceus77-7, enhanced by Donna
license: MIT
...
- `tmux` available for stable authenticated sessions during agent terminal calls (desktop app flow only)
...
## Agent Execution Pattern (desktop app flow)

Agent terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/donna-tmux-sockets"
mkdir -p "$SOCKET_DIR"
