...
metadata:
  hermes:
    tags: [skills, authoring, donna, conventions, skill-md]
...
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` in the agent package — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.

## When to Use

- User asks you to add a skill "in this branch / repo / commit"
- You're committing a reusable workflow that should ship with the agent package
- You're editing an existing skill under `skills/` in the package tree (use `patch` for small edits, `write_file` for rewrites; `skill_manage` still works for patch on in-repo skills, but not for `create`)
...
- Agent-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
