...
version: 2.0.0
author: Donna (writing-craft adapted from obra/superpowers)
license: MIT
...
Treat that as relative to the active working directory / backend workspace. Agent file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.
...
Use agent tools to understand the project:
