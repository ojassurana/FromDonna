...
metadata:
  hermes:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, kanban-video-orchestrator]
...
```
Donna -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to agent config (if missing)
...
3. **Restart agent session** to pick up the new MCP server
