...
version: 1.1.1
author: xdevplatform + openclaw + Donna
license: MIT
...
> **Docker HOME pitfall:** In the official agent Docker layout, `/opt/data` is `HERMES_HOME`, but agent tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for agent-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
- Agent adaptation: reformatted for skill conventions; safety guardrails preserved verbatim.
