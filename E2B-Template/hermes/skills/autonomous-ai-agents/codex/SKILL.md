...
metadata:
  hermes:
    ...
    related_skills: [claude-code, donna-agent]
---
...
For the agent itself, `model.provider: openai-codex` uses agent-managed Codex
...
## Agent Gateway Caveat

When invoking the Codex CLI from a the agent gateway/service context (for example,
