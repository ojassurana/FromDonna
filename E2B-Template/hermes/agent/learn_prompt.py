...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the agent
     skill-authoring standards (description <=60 chars, the modern section
     order, agent-tool framing, no invented commands).
...
Follow the agent skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Donna`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Donna.
...
Agent-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference agent tools by name in backticks: `terminal`, `read_file`,
