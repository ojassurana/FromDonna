...
distinct guidance for this case:
    ...
    "The model's thinking phase exceeded the upstream proxy's idle
     ...
     1. Set `providers.<provider>.models.<model>.stale_timeout_seconds: 900`
        in agent config (config.yaml under agent home) to extend the
        per-call timeout...
...
def build_thinking_timeout_guidance(
    ...
    return (
        ...
        "in agent config (config.yaml under agent home) to extend the per-call "
        "timeout. "
        "(the runtime's built-in floor is 600s for known reasoning models — "
